...
def _humidity_range_corroboration(
    ...
    (80-100%) is the sourced HIGH band. Corroboration reuses the
    source's OWN boundary number directly (never a separately invented
    one): for the low band, "matched" means current has dropped to or
    below the band's own upper edge; for the high band, "matched" means
    current has risen to or above the band's own lower edge.
    """
    row = (
        db.query(AgronomicParameter)
        .filter(
    ...
    boundary = row.value_max if low_band else row.value_min
    matched = current_value <= boundary if low_band else current_value >= boundary
    if matched:
        position = f"at or {'below' if low_band else 'above'} this band's own {'upper' if low_band else 'lower'} edge ({boundary}%)"
    else:
        position = f"{'above' if low_band else 'below'} this band's own {'upper' if low_band else 'lower'} edge ({boundary}%) -- not within the sourced band"
    note = (
        f"Sourced {parameter_name} range {row.value_min}-{row.value_max}% ({row.context}); "
        f"current value {current_value:.2f}% is {position}."
    )
    return matched, [note]
...
def _compute_severity(pa: ParameterAnalysis) -> tuple[str, SeverityFactors | None]:
    """
...
def compute_problem_assessment(
    ...
    if category.key == "heat_related":
        ...
        corroborated, corroboration_notes = _humidity_range_corroboration(
    ...
    severity, severity_factors = _compute_severity(pa)
